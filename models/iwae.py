...
class IWAE(nn.Module):
    ...
    def forward(self, x):
        ...
        z, dist = self.reparameterize(mu, logvar) # size of batch x num_particles x latent_size
        return self.decode(z), z, dist, mu, logvar

    def compute_loss(self, x, coeff=0.1, num_particles = 10, **kwargs):
        B, C, H ,W = x.size()
        ...
        loss = -torch.mean(logmeanexp(w, 1)) # size = 1
